 ...
 *
 *   average sold print            baselineLogHammer
 * + artist                        level + sum_j beta_j E[x_j] + E[house level] + E[year effect] - baseline
 * + signature / proof / edition / size / technique      sum over the attribute's columns of beta_j (x_j - E[x_j])
...
import { calibratedWitnesses, houseOffsetOf, type BlendCalibration } from "./knowledge_graph/price_blend.js";
...
const LABELS: Record<string, string> = { signature: "Signature", proof: "Proof", edition: "Edition", size: "Size", process: "Technique" };
const pretty = (s: string) => s.replace(/_/g, " ");

/** The lot's value on every model column, exactly as priorsModelPrediction reads them. */
...
function attrLabel(dim: string, ev: ValuationEvidence): string {
  const a = ev.attrs;
  ...
  switch (dim) {
    ...
    case "proof": return `${LABELS.proof}: ${pretty(a.proof.value)} (${src(a.proof)})`;
    case "edition": return `${LABELS.edition}: ${a.editionSize.value ?? "unknown"} (${src(a.editionSize)})`;
    case "size": return `${LABELS.size}: ${a.areaCm2.value != null ? `${Math.round(a.areaCm2.value)} cm²` : "unknown"} (${src(a.areaCm2)})`;
    case "process": return `${LABELS.process}: ${a.process.value} (${src(a.process)})`;
    default: return dim;
  }
}
...
export function valuationWaterfall(ev: ValuationEvidence, cal: BlendCalibration, means: ColumnMeans, medianGBP: number): Waterfall | null {
  ...
  const priors = calibratedWitnesses(inputs, cal, "no_estimate").witnesses.find((w) => w.source === "priors_model" && w.weight > 0) ?? null;
  const bars: WaterfallBar[] = [];
  let running = means.baselineLogHammer;
  ...
  push("baseline", `Average sold print (${means.trainRows.toLocaleString("en-GB")} auction sales)`, "baseline", 0);
  ...
  if (profile && priors) {
    ...
    const meanHouse = off ? Object.entries(means.houseShares).reduce((t, [h, s]) => t + s * houseOffsetOf(off, h).log, 0) : 0;
    const year = ev.valuationDate.value.slice(0, 4);
    const yearEff = profile.yearEffects[year] ?? 0;
    const keyBias = priors.mu - priors.rawMu; // key bias plus any house mix: everything calibration adds
    const artist = profile.level + betaMean + meanHouse + means.meanYearEffect - means.baselineLogHammer;
    push("artist", `Artist: ${ev.artist.canonical ?? ev.artist.reported ?? "unknown"} (${profile.basis === "shrunk" ? `own sales, ${profile.earlierSales}` : profile.basis === "prior" ? "similar artists' prices" : "segment default"})`, "factor", artist);
    for (const dim of ["signature", "proof", "edition", "size", "process"]) if (dim in byDim) push(dim, attrLabel(dim, ev), "factor", byDim[dim]);
    ...
    push("calibration", "Model calibration on realised hammers", "factor", keyBias);
    // The sum identity: whatever the pieces above miss is a bug, not a bar.
    const gap = priors.mu - running;
    if (Math.abs(gap) > 1e-6) {
      notes.push(`model bars fall ${gap.toFixed(4)} log short of the model price; folded into the artist bar`);
      const artistBar = bars.find((b) => b.key === "artist")!;
      artistBar.logEffect += gap; artistBar.multiplier = +Math.exp(artistBar.logEffect).toFixed(2);
      running = priors.mu;
    }
  ...
  const comps = calibratedWitnesses(inputs, cal, "no_estimate").witnesses.filter((w) => w.source !== "priors_model" && w.weight > 0);
  const compsLabel = comps.length
    ? `Market comps: ${comps.map((w) => pretty(w.source)).join(", ")}`
    : "No market comps";
  push("comps", compsLabel, "comps", Math.log(medianGBP) - running);
