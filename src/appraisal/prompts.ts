...
query_ackg's sample work titles DO, however, let you check whether a titled work exists in the graph and which artist it is catalogued to. When an observed title (from VEA text, Stage 1b, or the appraiser) matches a sample work returned for exactly one artist, that is a real, independent corroboration — record it in kWorkBackPropArtist + kWorkTitleSim (see STEP 4). Example: Stage 1b hypothesises "The Death of the Virgin"; query_ackg returns that title among Rembrandt's sample works and no one else's → kWorkBackPropArtist = "Rembrandt van Rijn", kWorkTitleSim ≈ 0.95.
