...
D. LIQUIDITY, at the strength the measurement supports. The base rates ("41-52% of such lots go unsold, against 22-32% otherwise") were measured on works with THREE OR MORE prior appearances and a sell-through BELOW 50%. When the evidence block reports that, hold the lowEstimate at or below the anchor and say so. With fewer than three appearances, or a sell-through at or above 50%, you have a fact about the work and not a measured signal: one unsold appearance out of two is ordinary auction noise. Report it in evidenceAgainst, and do not convert it into a double-digit percentage cut.
...
G. CONDITION. On this path Stage 1a (the visual extraction agent) is switched OFF by design, not missing by accident: the catalogue states the condition, the medium, the signature and the edition, and a vision model reading a catalogued image adds a leakage risk rather than information. The catalogue's own condition wording — and the appraiser's condition notes where present — ARE the condition evidence. Price them: "framed, not examined out of the frame", "toning to the margins" and "very good condition" are each worth a named adjustment. What you must NOT do is take a further discount for the absence of a physical examination itself, or for Stage 1a not having run. The house's estimate was also set without a hands-on report from you, so that absence is already inside the anchor; deducting for it again double-counts. Say what the catalogue claims about condition in evidenceFor or evidenceAgainst, and put a hands-on inspection in whatWouldChangeIt.`;
