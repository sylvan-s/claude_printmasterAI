...
- **K_work (Decision 9.1, Part B — embedding title matcher).** `ConceptualWork.name` is embedded once (`gemini-embedding-001`, 768-d, L2-normalized, `SEMANTIC_SIMILARITY`) via `backfill_title_embeddings.ts` → `titleEmbedding` property. `query_ackg_work` now takes `observedTitle` + `observedTechnique`; `scoreWorkTitleMatches` embeds the observed title, cosines against the catalogued vectors, rescales the compressed gemini cosine band (`titleSimFromCosine`, floor 0.72 / ceil 0.96) to a 0..1 `titleSim`, demotes technique-incompatible works, and returns the rows ranked. The agent transcribes the top row's `titleSim` / `matchedWorkTitle` / `backPropArtist`. New row **T8K**: a strong embedding match (`titleSim ≥ TAU_TITLE_ANCHOR` 0.85) identifies the Conceptual Work even when the title SOURCES give no consensus (the Hirst *Empresses* case). `titleSimFromCosine` + `title_normalize` tested in `npm run test:kg-parse`. Known limits: romaji↔English does not bridge (ukiyo-e), and two textually-near-identical different works (*Le Taureau* etching vs *Taureau* litho) score ~1.0 — the technique demotion + impression layer are the defence. Source-vs-source title clustering (`TAU_TITLE_AGREE`) is still token-based.
