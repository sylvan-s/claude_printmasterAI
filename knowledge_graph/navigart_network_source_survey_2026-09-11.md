...
### A fourth defect, from the crayon-manner re-run

Re-running the ingest for one vault (`--vault 24`, two records to fix) rewrote both
review CSVs from just that vault's rows: `navigart_unresolved_techniques.csv` went from
269 rows across 11 institutions to 7, and `navigart_excluded_records.csv` from 72 to 42.
Both truncations were committed before the line counts were read.

Same failure shape as the silent-zero domain filter in §4 — a partial result that looks
like a complete one. `run()` now writes scope-suffixed files
(`navigart_unresolved_techniques.v24.csv`) on any scoped run and says so, leaving the
network-wide files alone; only an unscoped run may write them. They are regenerated by
`--dry-run` over all vaults, which touches no graph.
