...
| `pricing.gbp_low_estimate`/`gbp_high_estimate` | — (UNMAPPED since BONHAMS-INGEST-1.1) | Not an estimate once a lot has sold: the API overwrites both with the sold price in GBP (equal to the hammer on all 39,851 sold Bonhams+Skinner rows, measured 2026-09-13), and on unsold rows the conversion is at a current rate. `SourceRecord.estimateLowGBP`/`estimateHighGBP` are derived from `estimates.low`/`high` at the ECB sale-date rate by `backfill_fx_gbp.py`; graph values written by 1.0 were corrected by `repair_bonhams_estimate_gbp.py` and are guarded by `check_bonhams_estimate_gbp.py` |
